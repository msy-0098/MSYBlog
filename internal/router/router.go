...
func New(deps Dependencies) *gin.Engine {
	...
	api.POST("/posts/:slug/comments", limiter.Limit(20, time.Minute), middleware.RequireAuth(deps.Config.Auth.JWTSecret, deps.Database), commentHandler.CreatePostComment)
	...
	api.POST("/admin/login/2fa", limiter.Limit(10, time.Minute), adminAuthHandler.Login2FA)

	admin := api.Group("/admin", middleware.RequireAdmin(deps.Config.Auth.JWTSecret, deps.Database), middleware.RequireCSRF())
	...
	admin.POST("/2fa/setup", adminAuthHandler.Setup2FA)
	admin.POST("/2fa/enable", adminAuthHandler.Enable2FA)
	admin.POST("/2fa/disable", adminAuthHandler.Disable2FA)
	...
	admin.POST("/posts", limiter.Limit(30, time.Minute), adminContentHandler.CreatePost)
	...
	admin.POST("/categories", limiter.Limit(30, time.Minute), adminContentHandler.CreateCategory)
	...
	admin.POST("/tags", limiter.Limit(30, time.Minute), adminContentHandler.CreateTag)
	...
	admin.POST("/projects", limiter.Limit(30, time.Minute), adminContentHandler.CreateProject)
	...
	admin.POST("/links", limiter.Limit(30, time.Minute), adminContentHandler.CreateLink)
	...
	admin.POST("/upload", limiter.Limit(20, time.Minute), adminContentHandler.UploadImage)
